...
# 75th percentile for all wages in Portland MSA.
...
## Create a new variable "inclevel" for each occupation that translates the median income of 
## that occupation into one of four categories: Low, Lower Middle, Upper Middle and High.
...
OccupationList$Occupation.title[is.na(OccupationList$median)] 
# This gives us the list of occupations for which we do not have median annual wage to determine their wage category.

### Below we manually assign "inclevel" values for those occupations that we do not have annual 
### median income. (Based on info that Nick sent by email on July 21, 2017)
...
for(i in 1:length(list1)) {
  ...
  output$industry[i] <- as.character(IndustryList$Industry[IndustryList$NAICS==list1[i]])
